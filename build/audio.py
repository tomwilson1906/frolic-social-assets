#!/usr/bin/env python3
"""Original upbeat audio bed for Frolic reels - fully synthesised, no licensing risk.
118 BPM, C major pentatonic marimba over a four-on-the-floor kick, clap, offbeat hats, bass.
API-published reels cannot use Instagram's trending library anyway, and baked-in licensed
music risks content-ID muting on every platform, so the bed is generated from scratch."""
import numpy as np, sys, wave

SR = 44100; BPM = 118.0; BEAT = 60.0 / BPM; BARS = 20
total = BARS * 4 * BEAT
N = int(total * SR)
...
def marimba(freq, dur, vel=1.0):
    ...
    s = (np.sin(2*np.pi*freq*tt)
         + 0.35*np.sin(2*np.pi*freq*3.9*tt)*np.exp(-tt*18)
         + 0.15*np.sin(2*np.pi*freq*9.2*tt)*np.exp(-tt*30))
