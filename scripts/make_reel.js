...
const F = p => `file://${path.resolve(__dirname, 'node_modules', p)}`;
const FONTS = `
...
:root{--cream:#FFF4E4;--paper:#FFFDF8;--pink:#FF3E8A;--pinktint:#FFE1EE;--marigold:#FFB627;--plum:#3D0C3A;--plumsoft:#8A5B86;--green:#2FA36B}
...
function confetti(idx, onMarigold) {
  const r = seeded(idx + 7); const cols = onMarigold ? ['#FF3E8A', '#FFFDF8', '#3D0C3A'] : ['#FF3E8A', '#FFB627', '#3D0C3A', '#2FA36B'];
  let h = '';
  for (let i = 0; i < 16; i++) {
    const x = 30 + r() * 1000, y = 30 + r() * 1840, rot = -40 + r() * 80, c = cols[Math.floor(r() * cols.length)];
    const w = 16 + r() * 12, ht = 26 + r() * 14;
    if (x > 120 && x < 960 && y > 340 && y < 1500) continue; // keep content zone clean
...
function cardHTML(card, idx) {
  ...
  return `<!doctype html><html><head><style>${CSS}</style></head><body>${confetti(idx, false)}<div class="wordmark">frolic</div>${body}</body></html>`;
}
...
(async () => {
  ...
  for (let i = 0; i < spec.cards.length; i++) {
    const html = cardHTML(spec.cards[i], i);
    ...
    console.log('rendered card', i + 1, spec.cards[i].kind);
  }
  await browser.close();
})();
